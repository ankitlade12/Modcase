...
type ModCaseAppOptions = {
  ...
  reddit?: {
    ...
    updateWikiPage?(options: { subredditName: string; page: string; content: string; reason?: string }): Promise<unknown>;
  };
...
function formatWikiPage(subreddit: string, buckets: BucketSummary[], now = Date.now()): string {
  return [
    `# How r/${subreddit} moderates`,
    `_Auto-generated by ModCase on ${new Date(now).toISOString()}. Aggregate only - no usernames, moderator identities, or raw content._`,
    '',
...
export function createModCaseApp({
  ...
  app.post('/internal/menu/publish-wiki', async (c) => {
    const input = await c.req.json<MenuItemRequest & Record<string, any>>().catch(() => ({}));
    const subreddit = extractSubreddit(input, getSubredditName());

    if (!reddit?.updateWikiPage) {
      return c.json<UiResponse>({ showToast: 'ModCase cannot publish to the wiki because the wiki API is not available.' });
    }

    const settings = await loadSettings(subreddit);
    const buckets = await collectBucketSummaries(subreddit, settings);
    const content = formatWikiPage(subreddit, buckets);

    try {
      await reddit.updateWikiPage({ subredditName: subreddit, page: 'modcase', content, reason: 'ModCase: how we moderate update' });
    } catch (error) {
      console.error('[ModCase] wiki publish failed:', error);
      return c.json<UiResponse>({ showToast: 'ModCase could not update the wiki page. Confirm the app is allowed to edit the wiki.' });
    }

    return c.json<UiResponse>({ showToast: `ModCase published the "how we moderate" summary to the r/${subreddit} wiki (page: modcase).` });
  });
